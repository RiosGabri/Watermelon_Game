...
// O PostStep agora apenas consolida os dados na fila global de fusões de forma segura
...
cpBool callbackFusao(cpArbiter* arbiter, cpSpace* espaco, cpDataPointer userData) {
    ...
    // Evita duplicar o agendamento da mesma colisão
    ...
    // Agenda para o Chipmunk processar logo após o término do cálculo físico atual
...
void registrarFusoes(cpSpace* espaco) {
    for (int i = 0; i < NIVEIS_FRUTA; i++) {
        cpCollisionHandler *handler = cpSpaceAddCollisionHandler(espaco, i, i);
        handler->beginFunc = callbackFusao;
    }
}